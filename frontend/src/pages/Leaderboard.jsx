...
export default function Leaderboard() {
  ...
  return (
    <div className="min-h-screen p-6 max-w-5xl mx-auto bg-obsidian">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10 pb-4 border-b border-myth-gold/30">
         <h1 className="text-3xl md:text-4xl font-myth font-bold tracking-widest flex items-center gap-4 text-glow-gold">
          <span className="text-myth-gold">VULNI</span><span className="text-myth-red">X</span><span className="text-myth-gold/60 text-lg md:text-2xl ml-1">LEADERBOARD</span>
         </h1>
         <button onClick={() => navigate('/dashboard')} className="w-full md:w-auto px-5 py-2 bg-myth-darker text-gray-300 rounded font-body uppercase hover:text-myth-gold transition-colors border border-gray-700 hover:border-myth-gold text-sm tracking-widest">
           Return
         </button>
      </header>
      ...
      <div className="bg-myth-dark/60 backdrop-blur border border-myth-gold/20 rounded-lg overflow-hidden shadow-myth-gold p-4 relative">
        ...
        {/* Animated header row */}
        <div className="hidden md:grid grid-cols-12 text-myth-gold/80 font-myth text-sm tracking-widest uppercase border-b border-myth-darker py-4 px-6 relative font-bold">
          <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-myth-gold/30 to-transparent"></div>
          <div className="absolute bottom-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-myth-gold/30 to-transparent"></div>
          
          <div className="col-span-2 text-center">Hierarchy</div>
          <div className="col-span-6">Deity Name</div>
          <div className="col-span-2 text-center">Merit</div>
          <div className="col-span-2 text-center">Vanquished</div>
        </div>
        ...
        <div className="space-y-3 mt-6 relative z-10">
          {!isReleased ? (
            <div className="text-center py-20 text-myth-red font-myth tracking-[0.2em] uppercase text-2xl animate-pulse group cursor-default">
             ...
             if (index === 0) {
               rowStyle = "bg-myth-gold/10 border-myth-gold/50 shadow-myth-gold hover:bg-myth-gold/20";
               rankStyle = "text-myth-gold text-glow-gold";
               rankIcon = '🐒 1';
             } else if (index === 1) {
               rowStyle = "bg-gray-300/10 border-gray-400/50 hover:bg-gray-300/20";
               rankStyle = "text-gray-300";
             } else if (index === 2) {
               rowStyle = "bg-orange-900/20 border-orange-700/50 hover:bg-orange-900/30";
             ...
             return (
               <div key={team.id} className={`flex flex-col md:grid md:grid-cols-12 items-center py-4 px-6 rounded border transition-all duration-300 hover:scale-[1.02] animate-slide-up gap-4 md:gap-0 ${rowStyle}`} style={{animationDelay:`${index * 0.1}s`,animationFillMode:'backwards'}}>
                 ...
                 <div className="md:col-span-6 font-body font-bold text-base md:text-lg text-gray-200 group flex items-center justify-center md:justify-start tracking-wider">
